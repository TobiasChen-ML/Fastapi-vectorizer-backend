...
def png2svg(imgaddrs):
    ...
    for imgaddr in imgaddrs:
        ...
        contours, hierarchy = cv2.findContours(
            cv2.cvtColor(image, cv2.COLOR_BGR2GRAY), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        ...
        # 最大值叠加  
        # image = np.maximum(array1, array2)  
        
        # 最小值叠加  
        image = np.minimum(array1, array2)
        ...
        t2 = time.time()
        timeconsuming.append((len(contours),t2-t1))
            #  mymask = []
            # maskcounter = 0
    ...
    import csv
    with open(f'output_{time.time()}.csv', 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        # 如果想加表头，可以写一行，例如：
        writer.writerow(['name', 'age'])
        
        # 写入所有双元组
        writer.writerows(timeconsuming)
...
if __name__ == "__main__":
    ...
    imgaddrlist = glob.glob(r'F:\Logo\logo2\*.png')
    my_new_list = split_list(imgaddrlist, 10)
    process_list = []
