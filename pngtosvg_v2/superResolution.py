...
# 重参数
class RepRFB(nn.Module): # Residual Feature Block
...
class ESA(nn.Module):
    def __init__(self, esa_channels, n_feats, conv): # 16, 48, nn.Conv2d
...
class super_resolution_predict:
    def __init__(self,model_path) -> None:
        self.model = RepRFN(deploy=True, upscale_factor=4)
        self.model_path = model_path
        self.model.load_state_dict(torch.load(self.model_path,map_location=torch.device('cpu')), strict=True)
        self.model.eval()
        for k, v in self.model.named_parameters():
            v.requires_grad = False
        self.model = self.model.to("cpu")

    def forward(self,logo):
        # logo = imread_uint(logo, n_channels=3)
        logo = uint2tensor4(logo,1.0)
        logo = logo.to("cpu")
        logo = self.model(logo)
        logo = tensor2uint(logo,1.0)
        return logo
